...
for i, r in enumerate(restaurants):
    ...
    article_docs.append(Document(page_content = text.strip(),
...
for i, (p, rec) in enumerate(zip(image_paths, recipes)):
    ...
    image_docs.append(Document(page_content = rec.get("name", f"recipe image {i}"),
                               metadata = {"doc_id": doc_id,
                                           "image_path": p,
                                           "source": "recipe_image",
...
article_db = Chroma(collection_name = "restaurant_articles",
                    persist_directory = DB_DIR)

article_db._collection.upsert(ids = [d.metadata["doc_id"] for d in article_docs],
                              embeddings = A.tolist(),
...
image_db = Chroma(collection_name = "food_images",
                  persist_directory = DB_DIR)

image_db._collection.upsert(ids = [d.metadata["doc_id"] for d in image_docs],
                            embeddings = V.tolist(),
