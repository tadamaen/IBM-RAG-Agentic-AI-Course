...
if not os.path.isdir(DB_DIR):
    raise RuntimeError(f"Vector database directory not found: '{DB_DIR}'. "
...
if n_articles <= 0 or n_images <= 0:
    raise RuntimeError("One or more collections are empty. Please rerun Lesson 1 to rebuild the index.")
...
def embed_texts(texts, batch_size = 64):
    return text_model.encode(texts, batch_size = batch_size, show_progress_bar = False, normalize_embeddings = True).astype(np.float32)
...
def retrieve_articles(query: str, k: int = 5, where: dict | None = None):
    ...
    res = article_db._collection.query(query_embeddings = [q_vec.tolist()],
                                       n_results = k,
                                       where = where,
...
def retrieve_images_by_text(query: str, k: int = 5, where: dict | None = None):
    ...
    res = image_db._collection.query(query_embeddings = [q_vec.tolist()],
                                     n_results = k,
                                     where = where,
                                     include = ["documents", "metadatas", "distances"])
    ids, docs, metas, dists = _unwrap(res)
...
def print_fused(rows, title: str, max_chars: int = 90):
    ...
    for idx, r in enumerate(rows, start = 1):
        ...
        if len(snippet) > max_chars:
            snippet = snippet[:max_chars].rstrip() + "..."
        print(f"[{idx}] {r['modality']} | id = {r['id']} | cuisine = {r['cuisine']} | "
...
rows = fuse_rank(q, k_text = 5, k_img = 5, w_text = 0.6, w_img = 0.4, where_text = None, where_img = None, top_n = 5)
...
rows = fuse_rank(q, k_text = 5, k_img = 5, w_text = 0.6, w_img = 0.4, where_text = where_articles, where_img = where_images, top_n = 5)
...
rows_text_heavy = fuse_rank(q, k_text = 5, k_img = 5, w_text = 0.8, w_img = 0.2, where_text = None, where_img = None, top_n = 5)
print_fused(rows_text_heavy, title = "Demo 3A — Text-heavy fusion (w_text = 0.8, w_img = 0.2)")
rows_image_heavy = fuse_rank(q, k_text = 5, k_img = 5, w_text = 0.3, w_img = 0.7, where_text = None, where_img = None, top_n = 5)
print_fused(rows_image_heavy, title = "Demo 3B — Image-heavy fusion (w_text = 0.3, w_img = 0.7)")
