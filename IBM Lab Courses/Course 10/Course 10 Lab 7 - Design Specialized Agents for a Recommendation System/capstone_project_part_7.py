...
# %pip install langchain==0.3.0 | tail -n 1
# %pip install langchain-community==0.3.27 | tail -n 1 
# %pip install langchain-core==0.3.74 | tail -n 1 
# %pip install langchain_experimental==0.3.4 | tail -n 1 
# %pip install openai==1.99.9 | tail -n 1
...
os.environ["OPENAI_API_KEY"] = "YOUR_API_KEY_HERE_FROM_OPENAI"
client = OpenAI()
MODEL = "gpt-5"

# Agent 1: User Profile Generator
user_profile_agent_config = {"role": "User Profile Generator",
                             "goal": """Analyze user restaurant visit history and social media posts to create a comprehensive profile including preferences, 
                             dietary restrictions, favorite cuisines, and dining patterns.""",
...
rag_retriever_agent_config = {"role": "RAG Retriever",
                              "goal": """Query multimodal vector databases to retrieve relevant restaurants, recipes, and food-related content based on user 
                                         profiles and similarity search.""",
...
food_trend_analyst_config = {"role": "Food Trend Analyst",
                             "goal": """Identify current food trends, popular ingredients, emerging dining concepts, and culinary movements to ensure 
                                        recommendations are timely and culturally relevant.""",
...
food_style_expert_config = {"role": "Food Style Expert",
                            "goal": """Analyze cuisine types, regional variations, cooking methods, and flavor profiles to match user preferences 
                                       with appropriate food styles""",
...
nutrition_expert_config = {"role": "Nutrition Expert",
                           "goal": """Evaluate nutritional content, identify allergens, assess dietary restrictions, and ensure recommendations align 
                                      with users' health and wellness goals.""",
...
recommendation_expert_config = {"role": "Recommendation Expert",
                                "goal": """Synthesize insights from all agents — user profiles, retrieved data, trends, food styles, and nutrition — into cohesive, 
                                           well-reasoned restaurant and recipe recommendations.""",
...
task_generate_profile = {"description": """Analyze the user's restaurant visit history and social media posts to create a comprehensive profile.
...
task_retrieve_candidates = {"description": """Based on the user profile, query the vector database to retrieve:
...
task_analyze_trends = {"description": """Analyze current food trends relevant to the retrieved restaurants and recipes. Identify:
...
print(f"Task: {task_analyze_trends['description'][:100]}...")
print(f"\nExpected Output: {task_analyze_trends['expected_output'][:100]}...")
print(f"\nAgent: {task_analyze_trends['agent']}")

# Task 4: Design the task for the Food Style Expert
task_analyze_food_styles = {"description": """Analyze the cuisine types, cooking techniques, and flavor profiles of the retrieved restaurants and recipes. Identify:
                                              - Dominant cuisine categories represented in the recommendations
                                              - Common flavor profiles (spicy, sweet, savory, umami, tangy, smoky, etc.)
                                              - Signature ingredients and cooking methods
                                              - Similarities and differences between restaurant offerings and recipes
                                              - Opportunities to introduce complementary or diverse food styles
                                              Explain how these food styles align with the user's preferences and dining behavior.""",

                            "expected_output": """A food style analysis with:
                                                  - List of cuisine categories and their frequency
                                                  - Breakdown of key flavor profiles and signature ingredients
                                                  - Summary of cooking techniques represented
                                                  - Comparison of restaurant and recipe food styles
                                                  - Recommendations for food styles the user may enjoy exploring
                                                  - Overall assessment of how well the retrieved options match the user's taste preferences""",
...
task_evaluate_nutrition = {"description": """Evaluate the nutritional aspects of the retrieved restaurants and recipes. Check for:
...
print(f"Task: {task_evaluate_nutrition['description'][:100]}...")
print(f"\nExpected Output: {task_evaluate_nutrition['expected_output'][:100]}...")
print(f"\nAgent: {task_evaluate_nutrition['agent']}")

# Task 6: Generate final recommendations
task_generate_recommendations = {"description": """Synthesize insights from all previous agents to generate final recommendations. Create:
...
# Testing The Individual Agents
sample_user_data = """Restaurant Visit History:
...
try:
    profile_result = test_agent(agent_prompt = user_profile_prompt,
...
agents_summary = [{"agent": "User Profile Generator", "task": "Generate User Profile"},
