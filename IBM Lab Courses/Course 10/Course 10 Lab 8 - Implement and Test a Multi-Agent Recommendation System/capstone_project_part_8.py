...
os.environ["OPENAI_API_KEY"] = "YOUR_API_KEY_FROM_OPENAI"
client = OpenAI()
MODEL = "gpt-5"

# Import agent configurations (from previous lesson)
agent_configs = {"user_profile_generator": {"role": "User Profile Generator",
                 ...
                 "rag_retriever": {"role": "RAG Retriever",
                 ...
                 "food_trend_analyst": {"role": "Food Trend Analyst",
                 ...
                 "food_style_expert": {"role": "Food Style Expert",
                 ...
                 "nutrition_expert": {"role": "Nutrition Expert",
                 ...
                 "recommendation_expert": {"role": "Recommendation Expert",
...
# Define the shared state structure as a dictionary - Every node reads from and writes to this state.
INITIAL_STATE = {
    # Input
    "user_input": "",
...
def node_generate_profile(state: dict) -> dict:
    ...
    user_message = f"""Analyze this user data and create a comprehensive profile: {state['user_input']}
...
def node_retrieve_candidates(state: dict) -> dict:
    ...
    user_message = f"""Based on this user profile: {json.dumps(profile, indent = 2)}
...
def node_analyze_trends(state: dict) -> dict:
    ...
    user_message = f"""Analyze current food trends in these options: Restaurants: {json.dumps(restaurants[:5], indent = 2)}
...
def node_analyze_styles(state: dict) -> dict:
    ...
    recipes = state["retrieved_recipes"]
    profile = state["user_profile"]
    user_message = f"""Analyze the cuisine types and flavor profiles of these retrieved options: User Profile: {json.dumps(profile, indent = 2)}
                                                                                                 Restaurants: {json.dumps(restaurants[:10], indent = 2)}
                                                                                                 Recipes: {json.dumps(recipes[:10], indent = 2)}

                       Identify:
                       - Dominant cuisine categories
                       ...
                       - Food style patterns that best match the user's preferences
                       - Complementary cuisines or flavor profiles the user may enjoy exploring

                       Return JSON: {{"cuisine_categories": [{{"name": str, "frequency": int}}],
                                      "flavor_profiles": [{{"name": str, "description": str}}],
                                      "signature_ingredients": [str],
                                      "cooking_techniques": [str],
                                      "style_matches": [str],
                                      "exploration_opportunities": [str],
                                      "summary": str}}"""
...
def node_evaluate_nutrition(state: dict) -> dict:
    ...
    recipes = state["retrieved_recipes"]
    profile = state["user_profile"]
    user_message = f"""Evaluate the nutritional fit of these options: User Profile: {json.dumps(profile, indent = 2)}
...
def node_generate_recommendations(state: dict) -> dict:
    ...
    user_message = f"""Synthesize these insights into top 5 restaurant and top 5 recipe recommendations: User Profile: {json.dumps(state['user_profile'], indent = 2)}
                       ...
                       Return JSON: {{"restaurants": [{{"name": str, "reasoning": str}}], "recipes": [{{"name": str, "reasoning": str}}]}}
...
def run_workflow(user_input: str) -> dict:
    """Run the full multi-agent workflow.
       Phases: 1. User Analysis (sequential)
...
test_user_1 = """Restaurant Visit History:
...
test_user_2 = """Restaurant Visit History:
