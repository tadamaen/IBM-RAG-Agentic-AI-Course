...
def main():
    try:
        ...
        collection.add(ids = [employee["id"] for employee in employees],
                       documents = employee_documents,
                       metadatas = [{"name" : employee["name"],
                                     "department" : employee["department"],
                                     "role" : employee["role"],
                                     "experience" : employee["experience"],
                                     "location" : employee["location"],
                                     "employment_type" : employee["employment_type"]} for employee in employees])
        ...
        def perform_advanced_search(collection, all_items):
            try:
                # Search Type 1: Similarity Search
                ...
                # Search Type 2: Metadata Filtering
                print("\n=== Metadata Filtering Examples ===")
                
                # Example 1: Filter by department
                print("\n1. Finding all Engineering employees:")
                ...
                print("\n2. Finding employees with 10+ years experience:")
                ...
                print("\n3. Finding employees in California:")
                results = collection.get(where = {"location" : {"$in" : ["San Francisco", "Los Angeles"]}})
                for i, doc_id in enumerate(results['ids']):
                    metadata = results['metadatas'][i]
                    print(f"- {metadata['name']}: {metadata['location']}")
        
                # Search Type 3: Combination Of Similarity Search And Metadata Filtering
                print("\n=== Combined Search: Similarity + Metadata Filtering ===")
        
                # Example: Find experienced Python developers (>= 8 years of working experience) in specific locations (San Francisco, New York and Seattle)
