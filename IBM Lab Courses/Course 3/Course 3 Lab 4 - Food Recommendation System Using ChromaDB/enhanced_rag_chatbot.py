...
def main():
    try:
        ...
        collection = create_similarity_search_collection("enhanced_rag_food_chatbot", {'description': 'Enhanced RAG chatbot with IBM watsonx.ai integration'})
...
def generate_llm_rag_response(query: str, search_results: List[Dict]) -> str:
    try:
        ...
        prompt = f'''You are a helpful food recommendation assistant. A user is asking for food recommendations, and I've retrieved relevant options from a food database.
                     User Query: "{query}"
                     Retrieved Food Information: {context}
...
# Function to generate fallback response when LLM fails"""
...
# Enhanced RAG-powered conversational food chatbot with IBM Granite (Main Chatbot Interface)
...
# [Optional] Enhanced comparison between two food queries using LLM
...
# [Optional] Generate AI-powered comparison between two queries
def generate_llm_comparison(query1: str, query2: str, results1: List[Dict], results2: List[Dict]) -> str:
    try:
        ...
        comparison_prompt = f'''You are analyzing and comparing two different food preference queries. Please provide a thoughtful comparison.
                                Query 1: "{query1}"
                                Top Results for Query 1: {context1}
                                Query 2: "{query2}"
                                Top Results for Query 2: {context2}
