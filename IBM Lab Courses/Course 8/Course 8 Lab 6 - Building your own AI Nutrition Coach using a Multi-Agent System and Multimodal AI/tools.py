...
credentials = Credentials(url = "https://us-south.ml.cloud.ibm.com", api_key = "YOUR_API_KEY_HERE")
client = APIClient(credentials)
project_id = "skills-network"

# Tool 1: Define The ExtractIngredientsTool Class
class ExtractIngredientsTool():
    ...
    def extract_ingredient(image_input: str):
        ...
        model = ModelInference(model_id = "meta-llama/llama-3-2-90b-vision-instruct",
                               credentials = credentials,
                               project_id = project_id,
                               params = {"max_tokens": 300})
      
        response = model.chat(messages = [{"role": "user", "content": [{"type": "text", "text": "Extract ingredients from the food item image"},
                                                                       {"type": "image_url", "image_url": {"url": "data:image/jpeg;base64," + encoded_image}}]}])

        return response['choices'][0]['message']['content']

# Tool 2: Define The FilterIngredientsTool Class
...
# Tool 3: Define The DietaryFilterTool Class
class DietaryFilterTool:
    ...
    def filter_based_on_restrictions(ingredients: List[str], dietary_restrictions: Optional[str] = None) -> List[str]:
        ...
        prompt = f"""You are an AI nutritionist specialized in dietary restrictions. 
                     Given the following list of ingredients: {', '.join(ingredients)}, and the dietary restriction: {dietary_restrictions}, 
        ...
        response = model.chat(messages = [{"role": "user", "content": [{"type": "text", "text": prompt}]}])
...
# Tool 4: Define The NutrientAnalysisTool Class
    ...
    def analyze_image(image_input: str):
        ...
        assistant_prompt = """You are an expert nutritionist. Your task is to analyze the food items displayed in the image and provide a detailed nutritional assessment using the following format:
                              ...
                              *   **Asparagus**: 3 spears, 25 calories
                              ...
                              Use bullet points, and for each nutrient provide details about the contribution of each food item.
                              Example:
        ...
        response = model.chat(messages = [{"role": "user",
