...
async def multi_agent_travel_planner_with_language():
    ...
    destination_expert = RequirementAgent(llm = llm,
                                          ...
                                          requirements = [ConditionalRequirement(ThinkTool, force_at_step = 1, min_invocations = 1, max_invocations = 5, consecutive_allowed = False),
                                                          ConditionalRequirement(WikipediaTool, only_after = [ThinkTool], min_invocations = 1, max_invocations = 4, consecutive_allowed = False)])
    
    # === AGENT 2: TRAVEL METEOROLOGIST ===
    travel_meteorologist = RequirementAgent(llm = llm,
                                            ...
                                            middlewares = [GlobalTrajectoryMiddleware(included = [Tool])],
                                            requirements = [ConditionalRequirement(ThinkTool, force_at_step = 1, min_invocations = 1, max_invocations = 2),
                                                            ConditionalRequirement(OpenMeteoTool, only_after = [ThinkTool], min_invocations = 1, max_invocations = 1)])
    
    # === AGENT 3: LANGUAGE & CULTURAL EXPERT ===
    language_and_culture_expert = RequirementAgent(llm = llm,
                                                   tools = [WikipediaTool(), ThinkTool()],
                                                   memory = UnconstrainedMemory(),
                                                   ...
                                                   middlewares = [GlobalTrajectoryMiddleware(included = [Tool])],
                                                   requirements = [ConditionalRequirement(ThinkTool, force_at_step = 1, min_invocations = 1, max_invocations = 3, consecutive_allowed = False)])
    
    # === AGENT 4: TRAVEL COORDINATOR (MAIN INTERFACE) ===
    
    handoff_to_destination = HandoffTool(destination_expert, name = "DestinationResearch",
    ...
    handoff_to_weather = HandoffTool(travel_meteorologist, name = "WeatherPlanning", 
    ...
    handoff_to_language = HandoffTool(language_and_culture_expert, name = "LanguageCulturalGuidance",
    ...
    travel_coordinator = RequirementAgent(llm = llm,
                                          ...
                                          instructions = """You are the Travel Coordinator, the main interface for comprehensive travel planning.
                                           ...
                                           middlewares = [GlobalTrajectoryMiddleware(included = [Tool])],
                                           requirements = [ConditionalRequirement(ThinkTool, consecutive_allowed = False),
