...
SYSTEM_PROMPT = SYSTEM_PROMPT = """You are Connoisseur Companion, an AI restaurant recommendation assistant.
                                   You have access to a database of California restaurants and can help users discover restaurants, explore dining options, and learn more about specific establishments.

                                   Available tools:
                                   1. get_restaurant_info
                                      - Use this tool to look up detailed information about a specific restaurant by name.
                                  
                                   2. recommend_by_vibe
                                      - Use this tool to find restaurants that match a particular mood, atmosphere, or vibe requested by the user.
                                  
                                   3. get_review
                                      - Use this tool to retrieve detailed reviews and customer feedback for a restaurant.

                                   Always use the appropriate tool when restaurant information is needed.
                                   Provide friendly, helpful, and informative responses based on the tool results."""

project_id = (os.environ.get("WATSONX_AI_PROJECT_ID") or os.environ.get("WATSONX_PROJECT_ID"))

# Initializing the WatsonX LLM
def make_model():
    return ChatWatsonx(model_id = "ibm/granite-4-h-small",
...
async def chat_with_agent(user_message: str, history: list) -> str:
    ...
    async with Client(transport) as client:
        ...
        openai_tools = [{"type": "function", "function": {"name": t.name, "description": t.description or "", "parameters": t.inputSchema}} for t in mcp_tools]
        ...
        for _ in range(10):
            ...
            if not response.tool_calls:
                ...
                if isinstance(raw, list):
                    return " ".join(b.get("text", "") if isinstance(b, dict) else str(b) for b in raw)
                return str(raw)
            ...
            for tool_call in response.tool_calls:
                ...
                tool_output = " ".join(item.text if hasattr(item, "text") else str(item) for item in result.content) if result.content else "(no result)"
...
with gr.Blocks(title = "Connoisseur Companion") as demo:
    ...
    msg_input = gr.Textbox(label = "Ask about restaurants",
