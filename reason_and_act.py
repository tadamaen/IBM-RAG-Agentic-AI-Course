...
os.environ["TAVILY_API_KEY"] = "YOUR_API_KEY_HERE" # fill in your API key using your Tavily account
...
# Building The Clothing Recommendation Tool
@tool
...
dummy_state: AgentState = {"messages": [HumanMessage( "What's the weather like in Zurich, and what should I wear based on the temperature?")]}
...
tool_message = ToolMessage(content = json.dumps(tool_result),
                           name = tool_call["name"],
...
# Building The Model Invocation Node
def call_model(state: AgentState):
    """Invoke the model with the current conversation state."""
    response = model_react.invoke({"scratch_pad": state["messages"]})
    return {"messages": [response]}

# Building The Decision Logic (Should Continue)
